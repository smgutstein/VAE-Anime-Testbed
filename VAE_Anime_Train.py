...
class VAE_Trainer:
    ...
    def __init__(self, config_file="config.ini"):
        ...
        # Constant idxs of test images / latent seeds for reproducible snapshots
        self.viz_rng = np.random.default_rng(self.seed + 1)
        self.fixed_test_img_idxs = self.viz_rng.choice(64, size=4, replace=False)

        tf_viz_gen = tf.random.Generator.from_seed(self.seed + 2)
        self.fixed_gen_img_seeds = tf_viz_gen.normal(shape=[4, self.vae.latent_dim])
    ...
    def load_config_file(self):
        ...
        if config.has_option('Training_Parameters', 'seed'):
            self.seed = int(config.get('Training_Parameters', 'seed'))
        else:
            self.seed = 1234

        if config.has_option('Training_Parameters', 'deterministic'):
            temp = config.get('Training_Parameters', 'deterministic').lower()
            self.deterministic = temp in ('true', '1', 'yes', 'y')
        else:
    ...
    def snapshot_vae_behavior (self, epoch=0, step=0, 
        ...
        rnd_test_img_idxs = np.random.choice(64, size=4, replace=False)
        ...
        # 4 are rndly chosen each time
        rnd_gen_img_seeds = tf.random.normal(
            shape=[4, self.vae.latent_dim],
            seed=self.seed + epoch * 100000 + step
        )
