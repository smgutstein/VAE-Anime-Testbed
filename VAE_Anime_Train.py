...
class VAE_Trainer:
    ...
    def __init__(
        ...
        *,
        ...
        self.reference_saver = (
            BestSSIMReferenceSaver(
                self.output_dir, self.cfg, resume=self.is_resuming
            )
    ...
    def _restore_for_resume(self, *, resume_checkpoint):
        ...
        # StepGuard's LR backoff lives on the optimizer; it must come back
        # with the checkpoint or the run silently resets to the configured LR.
        restored_lr = float(self.optimizer.learning_rate.numpy())
        expected_lr = float(payload["step_guard"]["min_learning_rate_seen"])
        # Relative tolerance: the optimizer stores LR as float32, StepGuard as
        # a Python float.
        if abs(restored_lr - expected_lr) > 1e-5 * abs(expected_lr):
            logging.warning(
                "Restored learning rate %.6g differs from checkpointed StepGuard "
                "learning rate %.6g",
                restored_lr,
                expected_lr,
            )
