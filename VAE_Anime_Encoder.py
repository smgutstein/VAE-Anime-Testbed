...
class Sampling(tf.keras.layers.Layer):
  def __init__(self, seed=0, **kwargs):
    ...
    # These variables control a stateless operation and are set before every
    # training step. Keep them out of model.weights so older .weights.h5 files
    # remain loadable after this reproducibility change.
  ...
  def call(self, inputs):
    ...
    dim = tf.shape(mu)[1]
    seed = tf.stack([
        tf.cast(self.base_seed, tf.int32) + self.epoch_seed,
        self.step_seed,
    ])
